...
{
    public class MapsRepositoryTests : EfRepositoryTests, IAsyncLifetime
    {
        ...
        public async Task GetByUserId_Empty_Success()
        {
            await using var unitOfWork = await UnitOfWorkFactory.CreateAsync();
            var repository = unitOfWork.GetRepository<IMapsRepository>();
            List<Map> storedMaps = await repository.GetByUserIdAsync(user.Id);
            await unitOfWork.CommitAsync();
        ...
        {
            Map[] maps = { CreateMap(user.Id), CreateMap(user.Id) };
